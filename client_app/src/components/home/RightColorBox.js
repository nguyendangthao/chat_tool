...
class RightColorBox extends Component {
    ...
    render() {
        return (
            <div className="col-md-4 offset-md-4 rightcolorbox">
                <ul className="panel" style={{ height: 500 }}>
                    {
                        this.state.colors.map((e, i) => {
                            return <li className={`item ` + (this.state.colorSelect === e ? 'active' : '')} key={i} >
                                <a style={{ backgroundColor: e }} key={e} href="/#" onClick={this._changeColor.bind(this, e)}>
                                </a>
                            </li>
                        })
                    }
